 ...
 * 86行 + 138行 + 206行
 */
public class SkinButton extends AppCompatButton implements ViewsMatch {
    ...
    public SkinButton(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray typedArray = context.obtainStyledAttributes(attrs,
                R.styleable.SkinButton,
                defStyleAttr, 0);
        // 存储到临时JavaBean对象
        attrsBean.saveViewResource(typedArray, R.styleable.SkinButton);
    ...
    public void startChangerSkin() {
        ...
        if (textColorResourceId > 0) {
            if (PluginManager.getInstance().isDefaultSkin()) {
                ...
                ColorStateList color = PluginManager.getInstance().getColorStateList(textColorResourceId);
                setTextColor(color);
            }
        }
